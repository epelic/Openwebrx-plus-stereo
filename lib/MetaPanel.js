...
function WfmMetaPanel(el) {
    ...
    this.fmLevelHandler = function(event) {
        ...
        $el.find('.rds-deviation').text('DEV: ≈ ' + (detail.deviation || 0).toFixed(1) + ' kHz');
        me.drawFmEqualizer(detail.audioBands || []);
    };
...
WfmMetaPanel.prototype.drawFmEqualizer = function(values) {
    var $bars = $(this.el).find('.rds-eq-bar b');
    $bars.each(function(index) {
        var value = Math.max(0, Math.min(1, values[index] || 0));
        $(this).css('height', Math.round(value * 100) + '%');
    });
};
...
function setFmBandwidth(input) {
    ...
    if (!demodulator || !demodulator.setBandpass) return;
    var half = khz * 500;
...
WfmMetaPanel.prototype.updateReceptionInfo = function() {
    ...
    if (typeof UI !== 'undefined' && UI.getDemodulator) {
        var demodulator = UI.getDemodulator();
        if (demodulator && Number.isFinite(demodulator.low_cut) && Number.isFinite(demodulator.high_cut)) {
    ...
    if (enabled) {
        $(this.el).removeClass('disabled').html(
            '<div class="rds-container">' +
                ...
                '<div class="rds-eq-title">AUDIO SPECTRUM</div><div class="rds-equalizer">' +
                    ['60','120','250','500','1k','2k','4k','8k','12k','15k'].map(function(label) { return '<div class="rds-eq-band"><i class="rds-eq-bar"><b></b></i><span>' + label + '</span></div>'; }).join('') +
                '</div><div class="fm-width"><span>STEREO WIDTH</span><input type="range" min="0" max="100" value="65" oninput="setFmStereoWidth(this)"><b class="fm-width-value">65%</b></div>' +
    ...
    var $container = $(
        '<div class="dab-container"><div class="dab-info-column">' +
            ...
            '<div class="dab-logo-column dab-station-column"><div class="dab-logo-title">DAB STATION</div>' +
                '<div class="dab-station-name">Seleziona un programma</div>' +
...
DabMetaPanel.prototype.updateDabStationInfo = function() {
    ...
    $(this.el).find('.dab-station-name').text(!name || name === 'Loading...'? 'Seleziona un programma' : name);
};
...
DabMetaPanel.prototype.clear = function() {
    ...
    $(this.el).find('.dab-station-name').text('Seleziona un programma');
