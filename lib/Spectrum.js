...
Spectrum.prototype.draw = function() {
    ...
    if (!this.ctx || spec_width!=this.el.width || spec_height!=this.el.height) {
        ...
        this.ctx.strokeStyle = "rgba(112, 255, 132, 1.0)";
...
//    }
    ...
    // Draw a single RF trace instead of a filled area.
    this.ctx.beginPath();
    if(spec_width <= data_width) {
        ...
        for(var x=0; x<spec_width; x++) {
            ...
            if (x === 0) this.ctx.moveTo(x, spec_height - y);
            else this.ctx.lineTo(x, spec_height - y);
        }
    } else {
        ...
        for(var x=0; x<data_width; x++) {
            ...
            var x_pos = x * x_ratio;
            if (x === 0) this.ctx.moveTo(x_pos, spec_height - y);
            else this.ctx.lineTo(x_pos, spec_height - y);
        }
    }
    this.ctx.stroke();
};
