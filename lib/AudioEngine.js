...
AudioEngine.prototype._start = function() {
    ...
    if (useAudioWorklets && me.audioContext.audioWorklet) {
        me.audioContext.audioWorklet.addModule('static/lib/AudioProcessor.js?v=dab-meter-mono-1').then(function(){
            ...
            me.audioNode.port.addEventListener('message', function(m){
                ...
                if (json.dabLevel) window.dispatchEvent(new CustomEvent('dab-audio-level', {detail: json.dabLevel}));
            });
...
AudioEngine.prototype.processAudio = function(data, resampler, recorder, stereo) {
    ...
    if (this.audioNode.port) {
        ...
        this.audioNode.port.postMessage({samples: buffer, stereo: !!stereo, forceMono: this.dabMono});
    } else {
        // silently drop excess samples
        if (this.getBuffersize() + buffer.length <= this.maxBufferSize) {
            this.audioBuffers.push({data: buffer, stereo: !!stereo, pos: 0});
        }
    }
}
...
AudioEngine.prototype.pushHdAudio = function(data) {
    var stereo = typeof UI !== 'undefined' && UI.getModulation && UI.getModulation() === 'dab';
    this.processAudio(data, stereo ? this.hdStereoResampler : this.hdResampler, this.hdRecorder, stereo);
    this.lastHd = true;
}
