function Filter(demodulator) {
    ...
    var wideAnalog = ['am', 'sam', 'data', 'usb', 'lsb', 'usbd', 'lsbd'].indexOf(demodulator.get_modulation()) >= 0;
    if (wideAnalog) max_bw = 15000;
    this.limits = { high: max_bw, low: -max_bw };
    this.max_passband = wideAnalog ? 30000 : null;
...
Envelope.prototype.draw = function(visible_range){
    ...
    if (!(to_px < 0 || from_px > window.innerWidth)) // out of screen?
    {
        ...
        if (fake_indicator) {
            ...
            // Keep the native envelope behavior, but make the two resize grips
            // easier to catch on compact and touch displays.
            drag_ranges.beginning = {x1: from_px - 8, x2: from_px + env_bounding_line_w + env_att_w + 8};
            drag_ranges.ending = {x1: to_px - env_bounding_line_w - env_att_w - 8, x2: to_px + 8};
...
Demodulator.prototype.setBandpass = function(bandpass) {
    var low = bandpass.low_cut;
    var high = bandpass.high_cut;
    if (this.filter.max_passband && high - low > this.filter.max_passband) {
        var center = (low + high) / 2;
        low = center - this.filter.max_passband / 2;
        high = center + this.filter.max_passband / 2;
    }
    this.low_cut = low;
    this.high_cut = high;
    this.set();
...
Demodulator.prototype.moveBandpass = function(low_new, high_new) {
    ...
    // Keep wide analog modes within the requested 15 kHz total passband.
    if (this.filter.max_passband && high_new - low_new > this.filter.max_passband) return;
