...
function WfmMetaPanel(el) {
    ...
    this.fmLevelHandler = function(event) {
        var value = Math.max((event.detail && event.detail.level) || 0, .001);
        var level = Math.max(0, Math.min(100, (60 + 20 * Math.log10(value)) * 100 / 60));
        $(me.el).find('.rds-vu-level').css('width', level + '%');
    };
    ...
    if (enabled) {
        $(this.el).removeClass('disabled').html(
            '<div class="rds-container">' +
                ...
                '</div>' +
                '<div class="rds-vu"><span>AUDIO</span><i><b class="rds-vu-level"></b></i></div>' +
...
WfmMetaPanel.prototype.clear = function() {
    ...
    $(this.el).find('.rds-vu-level').css('width', 0);
};
