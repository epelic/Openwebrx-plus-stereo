(function(){
  ...
  function retitle(){
    document.title='Max Mountain Station | Tactical SDR Console v5.2.1';
    var t=q('.webrx-rx-title');
    if(t){t.textContent='MAX MOUNTAIN STATION';t.setAttribute('title','MAX MOUNTAIN STATION')}
    var d=q('.webrx-rx-desc');
    if(d && !d.dataset.mmOriginal){d.dataset.mmOriginal=d.textContent||'';d.textContent='TACTICAL SDR RECEIVER • '+d.dataset.mmOriginal}
  }
  ...
  function buildWorkspace(){
    ...
    new MutationObserver(function(){
      window.clearTimeout(window.__mmRefreshTimer);
      window.__mmRefreshTimer=window.setTimeout(function(){pinReceiver();updateDock();normalizeButtons();removeUIKit();fixActionButtons()},80);
  ...
  function fixActionButtons(){
    qa('.openwebrx-record-button').forEach(function(b){
      b.classList.add('mm-action-button','mm-record-action');
      b.removeAttribute('style');
      b.title='Record audio';
    });
    qa('.button,[class*=button]').forEach(function(b){
      var text=((b.textContent||'')+' '+(b.title||'')).toLowerCase();
      if(/picture|screenshot|snapshot|capture/.test(text)){
        b.classList.add('mm-action-button','mm-picture-action');
        b.removeAttribute('style');
      }
    });
  }
  ...
  function init(){retitle();fixActionButtons();var tries=0,t=setInterval(function(){tries++;if(buildWorkspace()||tries>80){clearInterval(t);pinReceiver();normalizeButtons()}},100);removeUIKit();clock();setInterval(clock,1000);setInterval(function(){pinReceiver();removeUIKit();retitle();fixActionButtons()},1600);document.body.classList.add('mm-console-v52');setTimeout(forceResize,250);setTimeout(forceResize,1200)}
