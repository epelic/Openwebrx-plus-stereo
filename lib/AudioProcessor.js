class OwrxAudioProcessor extends AudioWorkletProcessor {
    constructor(options){
        ...
        this.peakLeft = 0;
        this.peakRight = 0;
        this.dabHeadroom = Math.pow(10, -6 / 20);
        this.playing = true;
        this.fadeGain = 1;
        this.lastLeft = 0;
        this.lastRight = 0;
        ...
        this.port.addEventListener('message', (m) => {
            if (typeof(m.data) === 'string') {
                ...
                this.forceMono = !!m.data.forceMono;
    ...
    process(inputs, outputs) {
        ...
        for (let i = 0; i < 128; i++) {
            ...
            const outputGain = this.fadeGain * (this.stereoMode ? this.dabHeadroom : 1);
            const left = this.leftBuffer[p] * outputGain;
            const right = this.rightBuffer[p] * outputGain;
            const mono = (left + right) * .5;
            output[0][i] = this.forceMono && this.stereoMode ? mono : left;
            output[1][i] = this.forceMono && this.stereoMode ? mono : right;
            this.peakLeft = Math.max(this.peakLeft, Math.abs(output[0][i]));
            this.peakRight = Math.max(this.peakRight, Math.abs(output[1][i]));
        ...
        if (this.levelFrames >= 2048) {
            this.port.postMessage(JSON.stringify({dabLevel: {left: this.peakLeft, right: this.peakRight}}));
            this.levelFrames = 0;
            this.peakLeft = 0;
            this.peakRight = 0;
        }
