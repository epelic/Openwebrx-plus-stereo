...
function DabMetaPanel(el) {
    ...
    var $container = $(
        '<div class="dab-container"><div class="dab-info-column">' +
            ...
            '</div></div>' +
            '<div class="dab-logo-column"><div class="dab-logo-title">STATION LOGO</div>' +
                '<div class="dab-logo-frame"><a class="dab-logo-link" target="_blank" rel="noopener noreferrer">' +
                '<img class="dab-logo-image" alt="Logo emittente DAB" /></a>' +
                '<span class="dab-logo-status">Seleziona un programma</span></div>' +
                '<div class="dab-logo-name"></div>' +
                '<div class="dab-logo-reception"><div class="dab-logo-block">BLOCCO DAB: --</div>' +
...
DabMetaPanel.prototype.updateLogoReceptionInfo = function() {
    ...
    $(this.el).find('.dab-logo-block').text('BLOCCO DAB: ' + (block? block[1].toUpperCase() : '--'));
    $(this.el).find('.dab-logo-signal').text('S/N: ' + (snr === '--'? snr : '≈ ' + snr) + ' · RF: ' + (rf || '--') + ' dB');
};

DabMetaPanel.prototype.updateStationLogo = function() {
    var me = this;
    var name = (this.$select.find('option:selected').text() || '').trim();
    if (!name || name === 'Loading...') return;
    var key = name.toLocaleLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
    var $image = $(this.el).find('.dab-logo-image');
    var $status = $(this.el).find('.dab-logo-status');
    var $link = $(this.el).find('.dab-logo-link');
    $(this.el).find('.dab-logo-name').text(name);
    $image.removeAttr('src').hide();
    $link.removeAttr('href');
    $status.text('Ricerca logo...').show();
    if (this.logoLookupTimer) clearTimeout(this.logoLookupTimer);
    var request = ++this.logoRequest;

    function show(station) {
        if (request !== me.logoRequest) return;
        if (!station || !station.favicon) {
            $status.text('Logo non disponibile').show();
            return;
        }
        var logo;
        try { logo = new URL(station.favicon, station.homepage || undefined); }
        catch (e) { $status.text('Logo non disponibile').show(); return; }
        if (!/^https?:$/.test(logo.protocol)) { $status.text('Logo non disponibile').show(); return; }
        $image.off('.dablogo').on('load.dablogo', function() {
            if (request !== me.logoRequest) return;
            $status.hide();$image.show();
        }).on('error.dablogo', function() {
            $image.removeAttr('src').hide();$status.text('Logo non disponibile').show();
        }).attr('src', logo.href);
        if (station.homepage) {
            try { var home = new URL(station.homepage); if (/^https?:$/.test(home.protocol)) $link.attr('href', home.href); }
            catch (e) {}
        }
    }

    if (Object.prototype.hasOwnProperty.call(this.logoCache, key)) {
        show(this.logoCache[key]);return;
    }
    this.logoLookupTimer = setTimeout(function() {
        var endpoint = 'https://de1.api.radio-browser.info/json/stations/search?name=' +
            encodeURIComponent(name) + '&countrycode=IT&limit=12&hidebroken=true&order=votes&reverse=true';
        fetch(endpoint, {headers: {'Accept': 'application/json'}}).then(function(response) {
            if (!response.ok) throw new Error('HTTP ' + response.status);
            return response.json();
        }).then(function(stations) {
            if (request !== me.logoRequest) return;
            var candidates = (stations || []).filter(function(s) { return s && s.favicon; });
            var normalized = function(value) { return String(value || '').toLocaleLowerCase().replace(/[^a-z0-9]+/g, ' ').trim(); };
            candidates.sort(function(a, b) {
                var an = normalized(a.name), bn = normalized(b.name);
                var ar = an === key? 3 : (an.indexOf(key) >= 0 || key.indexOf(an) >= 0? 2 : 0);
                var br = bn === key? 3 : (bn.indexOf(key) >= 0 || key.indexOf(bn) >= 0? 2 : 0);
                return br - ar || (b.votes || 0) - (a.votes || 0);
            });
            me.logoCache[key] = candidates[0] || null;
            show(me.logoCache[key]);
        }).catch(function() {
            if (request === me.logoRequest) $status.text('Logo non disponibile').show();
        });
    }, 350);
};
...
DabMetaPanel.prototype.clear = function() {
    ...
    this.logoRequest++;
    $(this.el).find('.dab-logo-image').removeAttr('src').hide();
    $(this.el).find('.dab-logo-link').removeAttr('href');
    $(this.el).find('.dab-logo-name').empty();
    $(this.el).find('.dab-logo-status').text('Seleziona un programma').show();
    this.updateLogoReceptionInfo();
};
