...
AudioEngine.prototype._start = function() {
    ...
    if (useAudioWorklets && me.audioContext.audioWorklet) {
        me.audioContext.audioWorklet.addModule('static/lib/AudioProcessor.js?v=fm-stereo-mpx-1').then(function(){
...
AudioEngine.prototype.processAudio = function(data, resampler, recorder, stereo, dab) {
    ...
    if (stereo) {
        ...
        if (dab) {
            ...
            window.dispatchEvent(new CustomEvent('fm-audio-level', {detail: {
                ...
                deviation: Math.min(99.9, Math.max(leftPeak, rightPeak) / 32768 * 75),
    ...
    if (this.audioNode.port) {
        ...
        this.audioNode.port.postMessage({samples: buffer, stereo: !!stereo, dab: !!dab, fm: !!stereo && !dab,
            forceMono: (dab && this.dabMono) || (stereo && !dab && this.fmMono)});
    } else {
        // silently drop excess samples
        if (this.getBuffersize() + buffer.length <= this.maxBufferSize) {
            this.audioBuffers.push({data: buffer, stereo: !!stereo, dab: !!dab, fm: !!stereo && !dab, pos: 0});
        }
    }
}
...
AudioEngine.prototype.pushHdAudio = function(data) {
    ...
    var stereo = modulation === 'dab' || modulation === 'wfm';
    this.processAudio(data, stereo ? this.hdStereoResampler : this.hdResampler, this.hdRecorder, stereo, modulation === 'dab');
    this.lastHd = true;
}
...
AudioRecorder.prototype.record = function(samples, stereo) {
    if (stereo) {
        // Recording remains a compatible mono MP3 while live DAB playback is stereo.
        var mono = new Int16Array(Math.floor(samples.length / 2));
        for (var j = 0; j < mono.length; j++) mono[j] = (samples[j * 2] + samples[j * 2 + 1]) / 2;
        samples = mono;
    }
